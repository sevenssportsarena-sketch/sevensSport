...
export default async function StandingsPage({
  ...
  const selectedLeagueId =
    leagueParam || (leagues.length > 0 ? leagues[0].id : undefined);
    ...
    <div className="container mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 space-y-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-border pb-6">
        <div className="space-y-4">
          ...
          <p className="text-muted-foreground max-w-2xl">
            Current standings, points, and goal differences for active football
            leagues.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-2">
          <h3 className="font-bold uppercase tracking-wider text-xs text-muted-foreground mb-4">
            Select League
          </h3>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-muted/10 text-muted-foreground border-b uppercase text-[11px] font-bold tracking-wider">
                    <tr>
                      ...
                      <th
                        className="px-4 py-4 text-center w-12"
                        title="Matches Played"
                      >
                        MP
                      </th>
                      <th className="px-4 py-4 text-center w-12" title="Won">
                        W
                      </th>
                      <th className="px-4 py-4 text-center w-12" title="Drawn">
                        D
                      </th>
                      <th className="px-4 py-4 text-center w-12" title="Lost">
                        L
                      </th>
                      <th
                        className="px-4 py-4 text-center w-12"
                      ...
                      <th
                        className="px-4 py-4 text-center w-16 text-primary"
                        title="Points"
                      >
                        Pts
                      </th>
                  ...
                  <tbody className="divide-y divide-border/50">
                    {standings.length === 0 ? (
                      ...
                      standings.map((row: any, index: number) => {
                        ...
                        const isEurope =
                          index >= 1 && index < Math.min(4, total - 3);
                        ...
                        return (
                          ...
                          >
                            <td className="py-3 text-center w-12 relative">
                              ...
                              <span
                                className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 rounded-r-full ${isChampions ? "bg-primary" : isEurope ? "bg-blue-500" : isRelegation ? "bg-red-500" : "bg-transparent"}`}
                              />
                              <span className="font-bold text-muted-foreground group-hover:text-foreground pl-4">
                                {index + 1}
                              </span>
                            </td>
                            ...
                            <td className="px-4 py-3 text-center font-black text-base text-primary">
                              {row.points}
                            </td>
                          </tr>
                        );
      ...
      <div className="flex flex-wrap gap-6 text-xs text-muted-foreground border-t border-border pt-6">
        ...
        <span className="ml-auto">
          MP = Played · W = Won · D = Drawn · L = Lost · GF = Goals For · GA =
          Goals Against · GD = Goal Difference · Pts = Points
        </span>
      </div>
    </div>
  );
